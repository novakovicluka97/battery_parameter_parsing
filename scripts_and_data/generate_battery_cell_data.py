...
# Parameters:
Ts = 1e-2                                    # SP execution rate (Should be maximum 1/SIMULATION_SPEED_UP)  # Todo static test fix because its hardcoded to work with ts = 1e-2
...
R1 = 0.64769e-3                              # 0.64769e-3 default but in Coursera quizzes more like 8 mili-ohm
C1 = 477.588e3/SIMULATION_SPEED_UP           # C is the only parameter that is now figuring in the RC time constant so it must be scaled
G = [67.207, 92.645, 67.840]                 # Todo Gamma parameter should depend on SIMULATION_SPEED_UP in some way
numpoles = 0
doHyst = 0
...
Ts = {str(Ts)}
Ts_cell = {str(Ts_cell)}
G = {str(G)}  # G param is wrong for default battery
